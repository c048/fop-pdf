...
class PDFBoxAdapter {
	...
	private static final Set filterFilter = new java.util.HashSet(Arrays.asList(new String[]{"Filter", "DecodeParms"}));
	private static final Set page2form = new java.util.HashSet(Arrays.asList(new String[]{"Group", "LastModified", "Metadata"}));
	...
	private void transferDict(COSDictionary orgDict, AbstractPDFStream targetDict, Set filter) throws IOException {
		transferDict(orgDict, targetDict, filter, false);
	}

	private void transferDict(COSDictionary orgDict, AbstractPDFStream targetDict, Set filter, boolean inclusive) throws IOException {
	...
	public PDFFormXObject createFormFromPDFBoxPage(PDDocument sourceDoc, PDPage page, String key) throws IOException {
		...
		form = pdfDoc.addFormXObject(null, pageStream, (pageResources != null ? pageResources.makeReference() : null), key);
